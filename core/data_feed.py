...
# Try importing yfinance as a universal fallback
try:
...
class DataFeed:
    ...
    def _detect_source(self, symbol: str) -> str:
        ...
        is_forex = (
            (any(p in symbol_upper for p in forex_patterns) and '/' not in symbol_upper)
            or symbol_upper.endswith('=X')
        )
    ...
    def _normalize_symbol_exchange(self, symbol: str) -> str:
        ...
        # Strip separators before matching (e.g. "BTC-USDT" → "BTCUSDT", "BTC_USDT" → "BTCUSDT")
        # This prevents "BTC-" being included in the base token name
        cleaned = symbol.replace('-', '').replace('_', '')

        # "BTCUSDT" → "BTC/USDT", "BTCUSD" → "BTC/USDT" (USD→USDT for exchange compat)
        stable_coins = ['USDT', 'BUSD', 'USDC', 'TUSD', 'DAI', 'USD']
        for sc in stable_coins:
            if cleaned.endswith(sc):
                base = cleaned[:-len(sc)]
                # Map bare USD to USDT (exchanges use USDT, not USD, as the quote asset)
                quote = 'USDT' if sc == 'USD' else sc
                return f"{base}/{quote}"

        return cleaned

    def _normalize_symbol_yfinance(self, symbol: str) -> str:
        ...
        # Crypto: strip stablecoin quote and append -USD
        crypto_quotes = ['USDT', 'BUSD', 'USDC', 'TUSD', 'DAI']
        for q in crypto_quotes:
            if s.endswith(q):
                base = s[:-len(q)]
                return f"{base}-USD"

        # Forex: pairs without a slash get =X suffix
        forex_currencies = ['EUR', 'GBP', 'JPY', 'CHF', 'AUD', 'NZD', 'CAD',
                             'SEK', 'NOK', 'DKK', 'SGD', 'HKD', 'MXN', 'ZAR',
                             'TRY', 'PLN', 'CZK', 'HUF']
        # If it's a 6-char pair of two known currencies, add =X
        if len(s) == 6:
            base3, quote3 = s[:3], s[3:]
            if base3 in forex_currencies or quote3 in forex_currencies:
                return f"{s}=X"

        # Already ends with USD (e.g. plain "BTCUSD")
        if s.endswith('USD') and len(s) > 3:
            base = s[:-3]
            return f"{base}-USD"

        # Pass through as-is (stocks, indices, ETFs)
        return s
    ...
    def _fetch_from_yfinance(self, symbol: str, timeframe: str,
        ...
        Works for crypto (BTC-USD), forex (EURUSD=X), stocks (AAPL),
        indices (^SPX), commodities (GC=F), and ETFs (SPY).
        No API key required.
        """
        if not YFINANCE_AVAILABLE:
            logger.warning("yfinance is not installed. Run: pip install yfinance")
            return None

        yf_symbol = self._normalize_symbol_yfinance(symbol)

        # Map internal timeframe codes to yfinance interval strings
        tf_map = {
            '1m': '1m',  '5m': '5m',  '15m': '15m', '30m': '30m',
            '1h': '60m', '2h': '90m', '4h': '1h',   '1d': '1d',
            '1w': '1wk', '1M': '1mo',
            'M1': '1m',  'M5': '5m',  'M15': '15m', 'M30': '30m',
            'H1': '60m', 'H2': '90m', 'H4': '1h',   'D1': '1d',
            'W1': '1wk',
        }
        ...
        logger.info(f"yfinance: downloading {yf_symbol} ({yf_interval}) from {start_date} to {end}")
        try:
            ticker = yf.Ticker(yf_symbol)
            df = ticker.history(start=start_date, end=end, interval=yf_interval, auto_adjust=True)
            ...
            # Drop yfinance-specific columns not used downstream
            df = df[['open', 'high', 'low', 'close', 'volume']]
            # Ensure DatetimeIndex has no timezone (consistent with CCXT output)
            if df.index.tz is not None:
                df.index = df.index.tz_localize(None)
            df.index.name = 'timestamp'

            logger.success(f"yfinance: fetched {len(df)} rows for {yf_symbol}.")
            return df

        except Exception as e:
            logger.warning(f"yfinance fetch error for {yf_symbol}: {e}")
            return None
