...
// const uchar * (fixed: matches uchar[] passed by meta())
...
//+------------------------------------------------------------------+
void ZmqMsg::getData(uchar &out[])
  {
   size_t sz=size();
   intptr_t src=data();
   if(ArraySize(out)<sz) ArrayResize(out,(int)sz);
   ArrayFromPointer(out,src);
  }
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
void ZmqMsg::setData(const uchar &in[])
  {
   intptr_t dest=data();
   size_t sz=size();
   ArrayToPointer(in,dest,(int)sz);
  }
//+------------------------------------------------------------------+
