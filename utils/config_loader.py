...
def load_all_configs(config_dir: str = None) -> Dict:
    ...
    Defaults to <project_root>/config/ resolved from this file's location,
    so the caller does not need to run from a specific working directory.
    """
    if config_dir is None:
        # utils/config_loader.py → project root is one level up
