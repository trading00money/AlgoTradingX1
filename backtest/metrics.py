...
def calculate_performance_metrics(
    ...
    metrics['Calmar Ratio'] = (
        annual_return / abs(drawdown.min())
        if drawdown.min() != 0
        else float('inf')
    )
    ...
    # Return raw numeric types — callers are responsible for formatting.
    # Replace any remaining numpy inf/nan with Python equivalents for safe JSON handling.
    import math
    clean = {}
